...
/* Returns true if the provided username exists and if the provided password is
   correct for that user, returns false otherwise. Undefined behavior in case of
   invalid input (NULL string). */
...
/* Adds a new user with the provided `username' and the provided password `pwd'
   (must not be NULL). Returns USER_SUCCESSFULLY_ADDED on success, USER_ALREADY_EXISTS
